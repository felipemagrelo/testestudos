...
DataManager._globalId       = 'AguaNaPedra'; /// alterado
...
/////////////////// A PARTIR DAQUI ESTOU TESTANDO AUTO-SIZE PERSONALIZADO///////

if (!window.debug) {
    console.log("isso é um teste :" + window.debug)
    Graphics._switchFullScreen();
}

let tileSize = 16;
let altu = screen.height;
let larg = screen.width;

let menor = larg > altu ? altu : larg;
let escala = menor < 9 * tileSize ?  1/2 : 1;

let hTiles = 0;
let vTiles = 0;

while (((escala*2)*tileSize)*9 < menor){
    escala = escala*2;
}

while (escala*tileSize*(vTiles+1)<= altu ){
    vTiles++
}
let vBorder = altu - (escala*tileSize*vTiles);

while (escala*tileSize*(hTiles+1)<= larg ){
    hTiles++
}
let hBorder = larg - (escala*tileSize*hTiles);



let wid = hTiles * tileSize;
let hei = vTiles * tileSize;

let winWid = wid*escala;
let winHei = hei*escala; 

if(window.debug){
    console.log(vBorder);
    console.log(hBorder);
    console.log("settings: "+ wid +", " + hei+", " + winWid+", " + winHei+", " + hTiles+", " + vTiles);
}

SceneManager._screenWidth       = wid;
SceneManager._screenHeight      = hei;
SceneManager._boxWidth          = winWid;
SceneManager._boxHeight         = winHei;
...
SceneManager.preferableRendererType = function() {
    if (Utils.isOptionValid('webgl')) {
        return 'webgl';
    } else if (Utils.isOptionValid('canvas')) {
        return 'canvas';
    } else {
        return 'auto';
    }
